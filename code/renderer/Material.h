...
#define VIRTUALTEXTURE_MAXMIPS		12
...
struct rvmVirtualMaterialOffset_t {
	short x;
	short y;
};
...
public:
	...
	idImage	*			LightFalloffImage() const { return lightFalloffImage; }
						...
						// Returns true if this mesh should have inlined collision.
	bool				InlineCollision() const { return inlineCollision; }
	...
	bool				HasMegaTexture() const { return hasMegaTexture; }

	int					GetVirtualTexturePageSource() const { return virtualTexturePageSource; }
	int					GetVirtualTextureWidthInPages(int mipMap = 0) const { return virtualTextureWidthInPages[mipMap]; }
	int					GetVirtualTextureHeightInPages(int mipMap = 0) const { return virtualTextureHeightInPages[mipMap]; }

	rvmVirtualImage		*GetVirtualAlbedoImage() const { return virtualAlbedoImage; }
	rvmVirtualImage		*GetVirtualNormalImage() const { return virtualNormalImage; }
	rvmVirtualImage		*GetVirtualSpecImage() const { return virtualSpecImage; }

	// Returns true if this material is virtual textured.
	bool				IsVirtualTextured() const { return virtualAlbedoImage != nullptr; }

	// Sets the virtual page offset for this material.
	void				SetVirtualPageOffset(int pageLOD, int pageX, int pageY, float virtualTextureX, float virtualTextureY);
...
private:
	...
	bool				hasMegaTexture;
	bool				inlineCollision;

	int					virtualTextureWidthInPages[VIRTUALTEXTURE_MAXMIPS];
	int					virtualTextureHeightInPages[VIRTUALTEXTURE_MAXMIPS];
	int					virtualTexturePageSource;

	int					virtualTexturePageOffsetNumMips;

	rvmVirtualImage		*virtualAlbedoImage;
	rvmVirtualImage		*virtualNormalImage;
	rvmVirtualImage		*virtualSpecImage;

	rvmVirtualMaterialOffset_t	*virtualPageOffsets[VIRTUALTEXTURE_MAXMIPS];

	textureStage_t		*albedoLitTextureStage;
	textureStage_t		*normalLitTextureStage;
	textureStage_t		*specularLitTextureStage;
