...
static infoParm_t	infoParms[] = {
	...
	{"lava",		0,	0,	CONTENTS_LAVA },		// used for lava
	{"slime",		0,  0,  CONTENTS_SLIME },		// used for slime
	{"fog",			0,  0,	CONTENTS_FOG  },		// used for fog
...
void idMaterial::ParseStage( idLexer &src, const textureRepeat_t trpDefault ) {
	...
	if (ss->lighting == SL_DIFFUSE) {
		albedoLitTextureStage = ts;
		albedoLitTextureStage->imagePath = imageName;
		ts->image = globalImages->defaultImage;
		return;
	}

	if (ss->lighting == SL_BUMP) {
		normalLitTextureStage = ts;
		normalLitTextureStage->imagePath = imageName;
		ts->image = globalImages->defaultImage;
		return;
	}

	if (ss->lighting == SL_SPECULAR) {
		specularLitTextureStage = ts;
		specularLitTextureStage->imagePath = imageName;
		ts->image = globalImages->defaultImage;
		return;
	}
...
===============
*/
void idMaterial::SetupVirtualTextureStages(void) {
	if (albedoLitTextureStage == nullptr)
		return;

	if (albedoLitTextureStage) {
		...
		if (albedoLitTextureStage->virtualImage)
		{
			...
			if (stupidVirtualPageSourceId >= MAXVIRTUALMATERIALS)
				stupidVirtualPageSourceId = 0;

			albedoLitTextureStage->image = virtualTextureSystem.GetVirtualAlbedoTexture();
			virtualTextureWidthInPages[0] = albedoLitTextureStage->virtualImage->GetWidthInPages(0);
			virtualTextureHeightInPages[0] = albedoLitTextureStage->virtualImage->GetHeightInPages(0);
			virtualTexturePageSource = stupidVirtualPageSourceId++;

			virtualTextureSystem.SetVirtualMaterial(virtualTexturePageSource, this);

			virtualAlbedoImage = albedoLitTextureStage->virtualImage;
			CreateVirtualPageOffsetsTexture();
		}
		else
		{
			return;
	...
	if (normalLitTextureStage) {
		idStr imageName = normalLitTextureStage->imagePath;

		idStr programFileName = va("generated/image_programs/%s_bump.tga", GetName());
		...
		if (normalLitTextureStage->virtualImage)
		{
		...
		{
			// In Doom 3 they did a lot of image combine work on the CPU side, which is a pain for us.
			// So we need to test if this is a image program, and save the result to generated so the virtual texture system can load it.
			byte *program_data;
			int program_width;
			int program_height;
			time_t program_timestamp;
			R_LoadImageProgram(imageName, &program_data, &program_width, &program_height, &program_timestamp);

			if (program_data == nullptr)
			{
				normalLitTextureStage->image = globalImages->defaultImage;
			}
			else
			{
				R_WriteTGA(programFileName, program_data, program_width, program_height);
				normalLitTextureStage->virtualImage = virtualTextureSystem.LoadVirtualImage(programFileName, TD_BUMP, albedoLitTextureStage->virtualImage->GetWidth(0), albedoLitTextureStage->virtualImage->GetHeight(0));
				virtualNormalImage = normalLitTextureStage->virtualImage;
				R_StaticFree(program_data);
			}
	...
	{
		memset(&defaultNormalLitTextureStage, 0, sizeof(defaultNormalLitTextureStage));
		normalLitTextureStage = &defaultNormalLitTextureStage;
	}
	...
	if (specularLitTextureStage) {
		idStr imageName = specularLitTextureStage->imagePath;

		specularLitTextureStage->virtualImage = virtualTextureSystem.LoadVirtualImage(imageName, TD_SPECULAR, albedoLitTextureStage->virtualImage->GetWidth(0), albedoLitTextureStage->virtualImage->GetHeight(0));
	...
	{
		memset(&defaultSpecularLitTextureStage, 0, sizeof(defaultSpecularLitTextureStage));
		specularLitTextureStage = &defaultSpecularLitTextureStage;
	}
	...
	if (virtualNormalImage == nullptr || virtualSpecImage == nullptr) {
		common->FatalError("Failed to load spec or normal for virtual texture interaction!");
	}
}
...
// jmarshall
		// Fix up our texture stage pointers.
		if (IsVirtualTextured())
		{
			for (int i = 0; i < numStages; i++)
			{
				if (stages[i].lighting == SL_DIFFUSE)
				{
					albedoLitTextureStage = &stages[i].texture;
				}

				if (stages[i].lighting == SL_BUMP)
				{
					normalLitTextureStage = &stages[i].texture;
				}

				if (stages[i].lighting == SL_SPECULAR)
				{
					specularLitTextureStage = &stages[i].texture;
				}
			}
		}
// jmarshall end
...
/*
===================
idMaterial::UpdateVirtualMaterialOffsetImage
===================
*/
void idMaterial::UpdateVirtualMaterialOffsetImage(void) const {
	virtualPageOffsetsImage->Bind();
...
/*
===================
idMaterial::SetVirtualPageOffset
===================
*/
void idMaterial::SetVirtualPageOffset(int pageLOD, int pageX, int pageY, float virtualTextureX, float virtualTextureY) {
	...
	vtOffset.y = virtualTextureY;
}
/*
===================
idMaterial::CreateVirtualPageOffsetsTexture
===================
*/
void idMaterial::CreateVirtualPageOffsetsTexture(void) {
	virtualTexturePageOffsetNumMips = 0;

	// Determine how many mips our page offset texture is going to have.
	int mipWidthInPages = virtualTextureWidthInPages[0];
	int mipHeightInPages = virtualTextureHeightInPages[0];
	while (true)
	{
		virtualPageOffsets[virtualTexturePageOffsetNumMips] = new rvmVirtualMaterialOffset_t[mipWidthInPages * mipHeightInPages];
		memset(virtualPageOffsets[virtualTexturePageOffsetNumMips], 0, mipWidthInPages * mipHeightInPages * sizeof(rvmVirtualMaterialOffset_t));
		
		if (mipWidthInPages == 1 || mipHeightInPages == 1)
		{
			virtualTexturePageOffsetNumMips++;
			break;
		}

		mipWidthInPages = mipWidthInPages / 2;
		mipHeightInPages = mipHeightInPages / 2;

		virtualTexturePageOffsetNumMips++;

		virtualTextureWidthInPages[virtualTexturePageOffsetNumMips] = mipWidthInPages;
		virtualTextureHeightInPages[virtualTexturePageOffsetNumMips] = mipHeightInPages;
	}

	// Create the virtual texture page offsets texture.
	idImageOpts opts;
	opts.format = FMT_RG16;
...
/*
===================
idMaterial::GetVirtualPageOffsetsImage
===================
*/
idImage	*idMaterial::GetVirtualPageOffsetsImage(void) const {
	idImage *image = virtualPageOffsetsImage;
