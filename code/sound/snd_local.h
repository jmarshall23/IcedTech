...
========================
*/
ID_INLINE_EXTERN uint32_t MsecToSamples( uint32_t msec, uint32_t sampleRate ) { return ( msec * ( sampleRate / 100 ) ) / 10; }
ID_INLINE_EXTERN uint32_t SamplesToMsec( uint32_t samples, uint32_t sampleRate ) { return sampleRate < 100 ? 0 : ( samples * 10 ) / ( sampleRate / 100 ); }
