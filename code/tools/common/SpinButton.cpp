...
{
	SetWindowLong ( hWnd, GWL_USERDATA, (INT_PTR)(inc * 100.0f) );
}

void SpinButton_SetRange ( HWND hWnd, float minRange, float maxRange )
{
	SendMessage ( hWnd, UDM_SETRANGE32, (INT_PTR)(minRange*100.0f), (INT_PTR)(maxRange*100.0f) );	
}
...
void SpinButton_HandleNotify ( NMHDR* hdr )
{
	...
	LONG minRange;
	LONG maxRange; 
	SendMessage ( hdr->hwndFrom, UDM_GETRANGE32, (INT_PTR)&minRange, (INT_PTR)&maxRange );
	if ( minRange !=  0 || maxRange != 0 )
	{
		float minRangef = (float)(INT_PTR)minRange / 100.0f;
