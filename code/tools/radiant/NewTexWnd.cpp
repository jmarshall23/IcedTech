...
int CNewTexWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) {
	...
	hdcTexture = radiantNetInterface.GetTexWndHandle();
	QEW_SetupPixelFormat(hdcTexture, false);
...
void CNewTexWnd::OnRender() {
	int nOld = g_qeglobals.d_texturewin.m_nTotalHeight;

	// jmarshall
	RECT    rcCli;
	::GetClientRect(WindowFromDC(radiantNetInterface.GetTexWndHandle()), &rcCli);
	rectClient.right = rcCli.right - rcCli.left;
	rectClient.bottom = rcCli.bottom - rcCli.top;
	// jmarshall end

	//hdcTexture = GetDC();
	if (!qwglMakeCurrent(radiantNetInterface.GetTexWndHandle(), win32.hGLRC)) {
		...
		globalImages->BindNull();
		glFinish();
		qwglSwapBuffers(radiantNetInterface.GetTexWndHandle());
		TRACE("Texture Paint\n");
	}
