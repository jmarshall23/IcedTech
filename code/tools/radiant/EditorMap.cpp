...
entity_t *EntityFromMapEntity(idMapEntity *mapent, RadiantProgressDialog *dlg) {
	entity_t *ent = NULL;
	if (mapent) {
		ent = Entity_New();
...
void Map_LoadFile(const char *filename) {
	...
	RadiantProgressDialog progressDialog(fileStr);
		...
		for (int i = 0; i < count; i++) {
			...
			if (mapent) {
				...
				if ( (GetTickCount() - lastUpdate) > 50 ) {
					...
					progressDialog.UpdateDialog(status,((float)i) / ((float)count) );
				}
				...
				if (classname == "worldspawn") {
					world_entity = EntityFromMapEntity(mapent, &progressDialog);
					Entity_PostParse(world_entity, &active_brushes);
				} else {
					ent = EntityFromMapEntity(mapent, &progressDialog);
	...
	progressDialog.UpdateDialog("Restoring Between");
	Map_RestoreBetween();

	progressDialog.UpdateDialog("Building Brush Data");
