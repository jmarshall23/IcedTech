...
/*
 ...
 =======================================================================================================================
 */
bool CDialogTextures::loadTree( HTREEITEM item, const idStr &name, RadiantProgressDialog *dlg, int &maxTextures, int &currentTexturesLoaded) {
	...
	if ( m_treeTextures.ItemHasChildren( item ) ) { 
		...
		while ( childItem != NULL ) {
			...
			if ( m_treeTextures.ItemHasChildren( childItem ) ) {
				if ( !loadTree( childItem, childName, dlg, maxTextures, currentTexturesLoaded) ) {
					return false;
				}
			} else {
				...
				if ( dw == TEXTURES || dw == MATERIALS ) {
					...
					float completed = (float)currentTexturesLoaded / (float)maxTextures;
					dlg->UpdateDialog(va("Loading Material %s", childName.c_str()), completed);
					Texture_ForName( childName );
					currentTexturesLoaded++;
				}
			}
...
// jmarshall
bool CDialogTextures::CountTexturesInTree(HTREEITEM item, idStr name, int &numTextures)
{
	if (item == NULL) {
		return true;
	}
	...
	if (m_treeTextures.ItemHasChildren(item)) {
		...
		while (childItem != NULL) {
			...
			if (m_treeTextures.ItemHasChildren(childItem)) {
				if (!CountTexturesInTree(childItem, childName, numTextures)) {
					return false;
				}
			}
...
// jmarshall
//	CWaitDlg dlg;
	RadiantProgressDialog dlg;
	int maxTextures = 0;
	int currentLoadedTextures = 0;
// jmarshall end
	Texture_HideAll();
...
// jmarshall
	// Figure out how many images are in this folder.
	CountTexturesInTree(item, name, maxTextures);

	loadTree(item, name, &dlg, maxTextures, currentLoadedTextures);
// jmarshall end
...
void CDialogTextures::addModels(bool rootItems) {
	...
	files = fileSystem->ListFilesTree( "models", ".ase|.lwo|.ma|.obj|.dae", true );
