...
//{{AFX_MSG_MAP(CMainFrame)
	...
	ON_COMMAND(ID_MEGATEXTURE_BUILDMEGATEXTURE, OnBuildMegaTexture)
...
void MFCCreate( HINSTANCE hInstance )
{
	...
	//LoadRegistryInfo("radiant2_SavedInfo", &g_qeglobals.d_savedinfo, &l);
	...
	int nOldSize = g_qeglobals.d_savedinfo.iSize;
	{
		...
		for (i=0 ; i<3 ; i++) {
			...
			g_qeglobals.d_savedinfo.colors[COLOR_GRIDBACK][i] = 0.77;
			g_qeglobals.d_savedinfo.colors[COLOR_GRIDMINOR][i] = 0.83;
			g_qeglobals.d_savedinfo.colors[COLOR_GRIDMAJOR][i] = 0.89;
			g_qeglobals.d_savedinfo.colors[COLOR_CAMERABACK][i] = 0.25;
		}

		g_qeglobals.d_savedinfo.colors[COLOR_GRIDBLOCK][0] = 0.0;
		g_qeglobals.d_savedinfo.colors[COLOR_GRIDBLOCK][1] = 0.65;
		...
		g_qeglobals.d_savedinfo.colors[COLOR_CLIPPER][0] = 1.0;
		g_qeglobals.d_savedinfo.colors[COLOR_CLIPPER][1] = 0.0;
		g_qeglobals.d_savedinfo.colors[COLOR_CLIPPER][2] = 0.0;

		g_qeglobals.d_savedinfo.colors[COLOR_BRUSHES][0] = 0.55;
		g_qeglobals.d_savedinfo.colors[COLOR_BRUSHES][1] = 0.02;
		g_qeglobals.d_savedinfo.colors[COLOR_BRUSHES][2] = 0.0;

		g_qeglobals.d_savedinfo.colors[COLOR_VIEWNAME][0] = 0.0;
		g_qeglobals.d_savedinfo.colors[COLOR_VIEWNAME][1] = 0.0;
		g_qeglobals.d_savedinfo.colors[COLOR_VIEWNAME][2] = 1.0;
...
void CMainFrame::OnDestroy() {
	...
	SaveWindowPlacement(radiantNetInterface.GetXYDialogHandle(), "radiant_xywindow");
	//SaveWindowPlacement(m_pXZWnd->GetSafeHwnd(), "radiant_xzwindow");
	//SaveWindowPlacement(m_pYZWnd->GetSafeHwnd(), "radiant_yzwindow");
	SaveWindowPlacement(radiantNetInterface.GetCameraDialogHandle(), "radiant_camerawindow");
...
// jmarshall - load in the entity decls before Radiant.Net initilization
	Eclass_InitForSourceDirectory(nullptr);
// jmarshall end
...
// jmarshall end
	...
	LoadWindowPlacement(radiantNetInterface.GetXYDialogHandle(), "radiant_xywindow");
	LoadWindowPlacement(radiantNetInterface.GetCameraDialogHandle(), "radiant_camerawindow");
	CreateQEChildren();
...
void CMainFrame::UpdateWindows(int nBits) {
	...
	if (nBits & W_CAMERA || ((nBits & W_CAMERA_IFON) && m_bCamPreview)) {
		...
		//}
		if (m_pCamWnd) {
			m_pCamWnd->RenderCamera();
		}
	//	radiantNetInterface.RedrawCameraWindow();
	}
...
void CMainFrame::OnBuildMegaTexture() {
	idStr megaTexturePath = currentmap;
	megaTexturePath.StripFileExtension();
	megaTexturePath.StripPath();

	idStr megaLightMap = currentmap;
	megaLightMap.StripFileExtension();

	cmdSystem->BufferCommandText(CMD_EXEC_NOW, va("megalight %s 8192\n", megaLightMap.c_str()));
	cmdSystem->BufferCommandText(CMD_EXEC_NOW, va("makemegatexture %s\n", megaTexturePath.c_str()));
