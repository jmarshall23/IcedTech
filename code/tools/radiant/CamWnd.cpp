...
void CCamWnd::RenderCamera() {
	bool		bPaint = true;

	if (!qwglMakeCurrent(radiantNetInterface.GetCameraWindowHandle(), win32.hGLRC)) {
		...
		Cam_Draw();
		QE_CheckOpenGLForErrors();
		qwglSwapBuffers(radiantNetInterface.GetCameraWindowHandle());
	}
}
...
int CCamWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) {
	HDC hDC = radiantNetInterface.GetCameraWindowHandle();
...
void CCamWnd::OriginalMouseUp(UINT nFlags, CPoint point) {
	RECT	r;
	::GetClientRect(WindowFromDC(radiantNetInterface.GetCameraWindowHandle()), &r);
...
// jmarshall
	RECT    rcCli;
	::GetClientRect(WindowFromDC(radiantNetInterface.GetCameraWindowHandle()), &rcCli);
	m_Camera.width = rcCli.right - rcCli.left;
	m_Camera.height = rcCli.bottom - rcCli.top;
// jmarshall end
...
void Select_ToCM() {
	CFileDialog dlgFile( FALSE, "lwo, ase", NULL, 0, "(*.dae)|*.dae|(*.obj)|*.obj|(*.lwo)|*.lwo|(*.ase)|*.ase|(*.obj)|*.obj||", g_pParentWnd );
...
// jmarshall
	refdef.skipFrustumInteractionCheck = true;
// jmarshall end
	...
	refdef.window_width = m_Camera.width;
