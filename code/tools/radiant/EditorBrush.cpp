...
void DrawProjectedLight(brush_t *b, bool bSelected, bool texture) {
	...
	idDict	spawnArgs;
	idRenderLightLocal fakeRenderLight;

	spawnArgs = ent->epairs;
	gameEdit->ParseSpawnArgsToRenderLight( &spawnArgs, &fakeRenderLight);
	R_RenderLightFrustum(fakeRenderLight.parms, planes );
...
// jmarshall - non selected meshes are now just white.
			idVec4 nonSelectedMesh;
			nonSelectedMesh.x = 1.0f;
			nonSelectedMesh.y = 1.0f;
			nonSelectedMesh.z = 1.0f;
			nonSelectedMesh.w = 1.0f;
