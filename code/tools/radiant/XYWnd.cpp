...
void CXYWnd::InitWnd()
{
	s_hdcXY = radiantNetInterface.GetXYWindowHandle();
	m_hWnd = WindowFromDC(s_hdcXY);
...
// jmarshall
	RECT    rcCli;
	::GetClientRect(WindowFromDC(radiantNetInterface.GetXYWindowHandle()), &rcCli);
	m_nWidth = rcCli.right - rcCli.left;
	m_nHeight = rcCli.bottom - rcCli.top;
// jmarshall end

	if (!qwglMakeCurrent(radiantNetInterface.GetXYWindowHandle(), win32.hGLRC)) {
	...
	if (bPaint) {
		...
		qwglSwapBuffers(radiantNetInterface.GetXYWindowHandle());
		TRACE("XY Paint\n");
	}
}
