...
float rvmBotAIBotActionBase::BotEntityVisibleTest(int viewer, idVec3 eye, idAngles viewangles, float fov, int ent, bool allowHeightTest) {
	...
	for (i = 0; i < 3; i++) {
		...
		if (i == 0)
			middle[2] += entinfo->GetPhysics()->GetBounds().b[0][2];// r.mins[2];
		else if (i == 1)
			middle[2] += entinfo->GetPhysics()->GetBounds().b[1][2] - entinfo->GetPhysics()->GetBounds().b[0][2];//entinfo->r.maxs[2] - entinfo->r.mins[2];
	}
	return bestvis;
}
