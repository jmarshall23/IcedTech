...
bool rvGEItemPropsTextPage::Init ( void )
{
	...
	SendMessage ( GetDlgItem ( mPage, IDC_GUIED_ITEMTEXTFONT ), CB_ADDSTRING, 0, (INT_PTR)"<default>" );
	...
	for ( i = 0; i < folders->GetNumFiles(); i++ ) {
		...
		SendMessage ( GetDlgItem ( mPage, IDC_GUIED_ITEMTEXTFONT ), CB_ADDSTRING, 0, (INT_PTR)folders->GetFile(i) );
	}
...
bool rvGEItemPropsTextPage::SetActive ( void )
{
	...
	font.StripQuotes ( );
	font.StripPath ( );
	fontSel = SendMessage ( GetDlgItem ( mPage, IDC_GUIED_ITEMTEXTFONT ), CB_FINDSTRING, -1, (INT_PTR)font.c_str () );
...
bool rvGEItemPropsTextPage::KillActive ( void )
{
	...
	if ( IsDlgButtonChecked ( mPage, IDC_GUIED_USETEXT ) )
	{
		...
		else
		{
			...
			SendMessage ( GetDlgItem ( mPage, IDC_GUIED_ITEMTEXTFONT ), CB_GETLBTEXT, fontSel, (INT_PTR)fontName );
...
bool GEItemPropsDlg_DoModal ( HWND parent, idWindow* window, idDict& dict )
{
	...
	propsp[RVITEMPROPS_GENERAL].lParam		= (INT_PTR)new rvGEItemPropsGeneralPage ( &dict, wrapper->GetWindowType ( ) );
	...
	propsp[RVITEMPROPS_IMAGE].lParam		= (INT_PTR)new rvGEItemPropsImagePage ( &dict );;
	...
	propsp[RVITEMPROPS_TEXT].lParam			= (INT_PTR)new rvGEItemPropsTextPage ( &dict );;
	...
	propsp[RVITEMPROPS_KEYS].lParam			= (INT_PTR)new rvGEItemPropsKeysPage ( &dict, wrapper );
