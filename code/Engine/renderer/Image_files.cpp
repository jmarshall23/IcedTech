...
================
*/
void R_WriteRaw(const char * filename, const byte *data, int width, int height) {
	idFileScoped file(fileSystem->OpenFileWrite(filename));
...
=============
*/
static void R_LoadPNG(const char* filename, unsigned char** pic, int* width, int* height, ID_TIME_T* timestamp)
{
	byte*	fbuffer;
...
Automatically attempts to load .png files if .tga files fail to load.
...
void R_LoadImage( const char *cname, byte **pic, int *width, int *height, ID_TIME_T *timestamp, bool makePowerOf2 ) {
	...
	if (ext == "png") {
		R_LoadPNG(name.c_str(), pic, width, height, timestamp);         
	}
	else if ( ext == "tga" ) {
		R_LoadTGA( name.c_str(), pic, width, height, timestamp );            // try tga first
	}
