...
bool idInventory::Give( idPlayer *owner, const idDict &spawnArgs, const char *statname, const char *value, int *idealWeapon, bool updateHud ) {
	...
	if ( !idStr::Icmpn( statname, "ammo_", 5 ) ) {
		...
		for( pos = value; pos != NULL; pos = end ) {
			...
			if ( common->IsMultiplayer() && weaponDecl && ( weapons & ( 1 << i ) ) && !weaponDecl->dict.GetInt( "ammoRequired" ) ) {
				continue;
			}
...
void idPlayer::Init( void ) {
	...
	if ( ( common->IsMultiplayer() || g_testDeath.GetBool() ) && skin ) {
		SetSkin( skin );
		renderEntity->SetShaderParms(6, 0.0f);
...
void idPlayer::Spawn( void ) {
	...
	if ( !common->IsMultiplayer() || entityNumber == gameLocal.localClientNum ) {
	...
	if ( common->IsClient() || entityNumber == gameLocal.localClientNum ) {
...
void idPlayer::UpdateSkinSetup( bool restart ) {
	...
	if ( gameLocal.gameType == GAME_TDM ) {
		...
		if ( !common->IsClient() && team != latchedTeam ) {
...
void idPlayer::NextBestWeapon( void ) {
	...
	if ( common->IsClient() || !weaponEnabled ) {
		return;
	}
...
void idPlayer::BobCycle( const idVec3 &pushVelocity ) {
	...
	if ( !physicsObj.HasGroundContacts() || ( common->IsMultiplayer() && spectating ) ) {
		// airborne
		bobCycle = 0;
		bobFoot = 0;
		bobfracsin = 0;
...
void idPlayer::UpdateViewAngles( void ) {
	...
	renderEntity->SetAxis(idAngles(0, viewAngles.yaw, 0).ToMat3());
...
void idPlayer::EvaluateControls( void ) {
	...
	if ( forceRespawn && !common->IsMultiplayer() && !g_testDeath.GetBool() ) {
...
void idPlayer::UpdateHud( void ) {
	...
	if ( isLagged && common->IsMultiplayer() && gameLocal.localClientNum == entityNumber ) {
...
void idPlayer::UpdateDeathSkin( bool state_hitch ) {
	...
	if ( health <= 0 ) {
		if ( !doingDeathSkin ) {
			...
			doingDeathSkin = true;
			//renderEntity->SetSha .noShadow = true;
			if ( state_hitch ) {
				renderEntity->SetShaderParms( SHADERPARM_TIME_OF_DEATH, gameLocal.time * 0.001f - 2.0f);
			} else {
		...
		renderEntity->SetShaderParms(SHADERPARM_TIME_OF_DEATH, 0.0f);
		UpdateVisuals();
		doingDeathSkin = false;
	}
}
...
void idPlayer::Think( void ) {
	...
	{
		idVec3 forward = renderEntity->GetAxis().ToAngles().ToForward();

		idVec3 playerLightRigOffset(0, 0, 90);

		playerLightRig1->SetOrigin((renderEntity->GetOrigin() + playerLightRigOffset) + (forward * 25));
		playerLightRig2->SetOrigin((renderEntity->GetOrigin() + playerLightRigOffset) - (forward * 25));
	}
...
void idPlayer::Killed( idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location ) {
	...
	if (health < -20) {
		gameLocal.CreateEffect("fx/gibs.fx", renderEntity->GetOrigin() + idVec3(0, 0, 40), renderEntity->GetAxis(), false);
	}
...
void idPlayer::SetInfluenceView( const char *mtr, const char *skinname, float radius, idEntity *ent ) {
	...
	if ( skinname && *skinname ) {
		...
		if ( head.GetEntity() ) {
			head.GetEntity()->GetRenderEntity()->SetShaderParms(SHADERPARM_TIMEOFFSET, -MS2SEC( gameLocal.time ));
		}
		UpdateVisuals();
	}
...
bool idPlayer::GetPhysicsToVisualTransform( idVec3 &origin, idMat3 &axis ) {
	...
	if ( common->IsClient() && gameLocal.framenum >= smoothedFrame && ( entityNumber != gameLocal.localClientNum || selfSmooth ) ) {
...
void idPlayer::UpdatePlayerIcons( void ) {
	int time = common->ServerGetClientTimeSinceLastPacket( entityNumber );
